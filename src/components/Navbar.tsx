...
const Navbar = () => {
    ...
    return (
        <div className="w-full h-20 bg-white border-b-[1px] border-b-gray-400 sticky top-0 z-50">
            ...
            {/* Mobile Menu */}
            {isMobileMenuOpen && (
                <div className="lg:hidden absolute top-20 left-0 w-full bg-white border-t-[1px] border-t-gray-400 shadow-lg">
                    <div className="flex flex-col items-center">
                        {navBarList.map((item) => (
                            <Link
                                href={item?.link}
                                key={item?.link}
                                className={`py-4 px-12 text-gray-600 text-lg hover:underline underline-offset-4 decoration-[1px] hover:text-gray-950 ${pathname === item?.link && "text-gray-950 underline"}`}
